...
// TODO: include source (stackoverflow?)

pub trait Permutations<T: Copy + Clone + Ord> {
    fn permutations(&self) -> UniquePermutations<T>;
}

impl<T: Copy + Clone + Ord> Permutations<T> for Vec<T> {
    fn permutations(&self) -> UniquePermutations<T> {
        UniquePermutations::new(self.to_vec())
    }
}

pub enum UniquePermutations<T: Copy + Clone + Ord> {
    Leaf {
